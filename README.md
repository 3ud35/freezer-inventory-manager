...
- Highlight items that need to be eaten soon (the look-ahead period is configurable, e.g., 60 days).
- Send a summary email every week at a configurable time (e.g., Friday at 7:00 PM).
- **Visual Alerting:** The Google Sheet itself includes conditional formatting to visually highlight items based on their expiration date directly in your table.

## Setup Instructions

1. **Prepare your Sheet:** Create a spreadsheet with at least two columns: one for the Item Name and one for the Expiration Date.
   - *Note:* You can view my [public spreadsheet](https://docs.google.com/spreadsheets/d/1llPgHkNjkhm_Z8hcYNVFVseVN_o0Vw24M4996GuxE6c/edit?usp=sharing) as an inspiration for the structure and conditional formatting rules, although the file is view-only to protect personal data.
...
4. **Configure:** Modify the `Configuration Constants` at the top of the script:
   ...
   - `ALERT_THRESHOLD_DAYS`: Number of days in advance to get notified (e.g., 60).
   ...
   - `COL_ITEM_NAME` & `COL_EXPIRATION_DATE`: Column indexes (e.g., Column A = 0, Column G = 6).
